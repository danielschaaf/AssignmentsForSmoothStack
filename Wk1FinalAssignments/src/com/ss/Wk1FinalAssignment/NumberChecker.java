 ...
 */
	...
	public String isPrime(Integer i) {
		StringBuffer s;
				...
				if (i <= 1) 
					...
					for (j=2; j <= Math.sqrt(k) && prime; j++) { //tests all factors of the number up to the squareroot of the number
	...
	public String isPalindrome(Integer i) {
		...
		while (!reversedDigits.isEmpty()) {  
		 ...
		 //subtracts the PALINDROME from the original number to check if it's a palindrome
		if (i.intValue() - p == 0)
			s.append("PALINDROME");
	 ...
	 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	 */
	private String processFunction(Integer i, Function_Chooser_Int f) { //local method to use Function_Chooser_Int interface
		return f.doesThis(i);
	}
	}
